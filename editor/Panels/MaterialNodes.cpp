...
ImU32 evaluate(const example::Graph<IMNode>& graph, const int root_node)
{
    ...
    while (!postorder.empty())
    {
        ...
        postorder.pop();
        const IMNode node = graph.node(id);
    ...
    void MaterialNodes::OnImGuiRender()
    {
        ...
        ImGui::Begin("Material Nodes", NULL, flags);
        ...
        // These are driven by the user, so we place this code before rendering the nodes
        {
            ...
            if (ImGui::BeginPopup("add node"))
            {
                ...
                {
                    const IMNode value(NodeType::value, 0.f);
                    const IMNode op(NodeType::add);
                ...
                {
                    const IMNode value(NodeType::value, 0.f);
                    const IMNode op(NodeType::multiply);
                ...
                {
                    const IMNode value(NodeType::value, 0.f);
                    const IMNode out(NodeType::output);
                ...
                if (ImGui::MenuItem("time"))
                {
                    ...
                    ui_node.id = graph_.insert_node(IMNode(NodeType::time));
